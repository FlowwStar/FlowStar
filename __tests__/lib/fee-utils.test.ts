...
describe('calculateFeeBreakdown', () => {
  ...
  it('returns correct component breakdown summing to total', () => {
    const result = calculateFeeBreakdown(100_000)
    const sum = result.networkFee + result.cpuFee + result.memoryFee + result.storageFee
    expect(sum).toBe(result.totalEstimated)
  })
  ...
  it('calculates estimatedUsd with custom xlmPrice', () => {
    ...
    expect(result.estimatedUsd).toBeCloseTo(0.5 * Math.ceil(10_000_000 * 1.15) / 1e7, 5)
  })

  it('networkFee is at least the base minimum', () => {
    const result = calculateFeeBreakdown(100)
    expect(result.networkFee).toBeGreaterThanOrEqual(100)
  })
})
